...
class Kemonotify:
    ...
    def __init__(self):
        ...
        self.placeholder = tk.Label(self.interior, text="TYPE TO SEARCH")
    ...
    def _start_sys_tray(self, options=()):  # root.withdraw(), root.deiconify()
    ...
    def _display_creators(self):
        ...
        state = self._pack_creators_get_state(creators)
        for c in state:
            if c.idn not in [creator.keys()[0].idn for creator in self.current_checker]:
                self.current_checker.append({c: state[c]})
                continue
            self.current_checker = [
                {creator: self.current_checker[creator]}
                if c.idn != creator.idn
                else {creator: self.current_checker[creator].set(tk.TRUE)}
                for creator in self.current_checker.keys()
            ]
    ...
    def _check_wanted(self):
        for creator in self.current_checker:
            if creator in self.will_check:
                if not self.current_checker[creator].get():
                    self.will_check.remove(creator)
                continue
